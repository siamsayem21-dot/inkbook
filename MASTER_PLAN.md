...
**Post-mission work, 2026-08-19 — Stripe Connect payment architecture.** With all 8 phases complete, Siam requested a plan (then approved it) to replace the deferred "1% platform fee" item (Phase 3/Phase 1 table row, DEFERRED_ISSUES.md #3) with InkBook's real business decision: subscription-only revenue, client deposit/remainder payments routed 100% directly to each studio's own Stripe account via Connect. All safe code preparation is complete, tested, and deployed inert behind a feature flag — see DEFERRED_ISSUES.md #3 and TASKS.md for the full detail and the manual activation checklist. This supersedes the Phase 1 table's "1% platform transaction fee: BLOCKED_NEEDS_SIAM" row below — that row is left as originally written (accurate for its time) rather than edited, per this file's own stated convention of not silently rewriting history.
