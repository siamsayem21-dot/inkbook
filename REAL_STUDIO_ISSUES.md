...
STATUS: ✅ FULLY FIXED AND VERIFIED (2026-08-24/25) — batch 1 (register/login/reset-password) committed as `6995af0`. Batch 2 (all 21 remaining files, plus `ConsultationForm.tsx` found missed from the original list during the repo-wide re-sweep) fixed in the follow-up "Final Cleanup + Focused Verification" pass. Full repo-wide `<label>` vs `htmlFor` sweep confirms every remaining gap is either a custom widget with no single associable native input (star-rating/category button-groups, color-swatch toggle — not fixable without a UI redesign, correctly out of scope), a valid wrapping-label pattern (checkbox/file-trigger `<label><input/>text</label>`), or intentionally-untouched dead code (see ISSUE-002). Verified via `tsc`/lint/production build/601 unit tests all clean, plus live `getByLabel()` regression checks against the newly-fixed fields (including the core public `ConsultationForm.tsx`).
LAUNCH BLOCKER: No — all flows remained completable by sighted users throughout; now also correctly accessible via label-based assistive tech/tooling.
...
CORRECTION (2026-08-24/25 pass): `components/booking/ConsentForm.tsx`'s "1 remaining unlinked label" (the aftercare/agreement checkbox) was never actually a bug — it uses the standard, fully valid implicit `<label><input type="checkbox"/>text</label>` wrapping pattern, which needs no `htmlFor`. No fix was needed or applied there.
...
FILES: app/(owner)/owner/artists/new/page.tsx; app/dashboard/artists/page.tsx; app/dashboard/bookings/page.tsx; app/dashboard/consent-forms/page.tsx; app/dashboard/consent-forms/_components/ConsentFormsTable.tsx; app/dashboard/_components/DashboardSidebar.tsx; app/dashboard/layout.tsx; app/(owner)/owner/artists/[artistId]/page.tsx; components/owner/ArtistTable.tsx
FIX: Not applied — deleting ~9 unreachable files is a judgment call outside a QA pass's "smallest safe fix" scope, same category as the already-deferred `app/client-portal/**` prototype cleanup (DEFERRED_ISSUES.md #4). Recommend bundling all three cleanups into one dedicated task.
TEST: N/A — not fixed.
...
UPDATE (2026-08-24/25 pass): 2 more orphaned files found while doing the accessibility label-fix sweep: `app/(owner)/owner/artists/[artistId]/page.tsx` (hardcoded fake stats "14 bookings"/"$3,200 revenue"/"2% no-show rate", unwired Save/Remove buttons — confirmed unreachable via repo-wide grep, zero real links to this route) and `components/owner/ArtistTable.tsx` (unused component, its own internal link points at a nonexistent `/artists/[id]` route that isn't even part of this dead file's own pair). Deliberately not fixed/deleted this pass either, per the same "dead-code cleanup is a bundled decision for Siam" reasoning, and per this pass's explicit instruction not to delete dead-code pages.
