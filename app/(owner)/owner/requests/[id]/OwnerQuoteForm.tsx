    ...
    <div className="bg-[#111] border border-[#1E1E1E] rounded-xl p-6 space-y-4">
        ...
        <form onSubmit={handleApprove} className="space-y-4">
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={loading}
              className="flex-1 bg-green-600 hover:bg-green-500 text-white font-bold text-sm py-3 rounded-lg disabled:opacity-50"
            >
              {loading ? "Approving…" : "Confirm Approval"}
            </button>
            <button type="button" onClick={() => setView(null)}
              className="px-5 py-3 border border-white/10 text-zinc-400 text-sm rounded-lg"
            >
              Cancel
            </button>
          </div>
      ...
      {view === "decline" && (
        <form onSubmit={handleDecline} className="space-y-4">
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={loading}
              className="flex-1 bg-red-700 hover:bg-red-600 text-white font-bold text-sm py-3 rounded-lg disabled:opacity-50"
            >
