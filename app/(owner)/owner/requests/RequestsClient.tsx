...
function statusClass(s: string) {
  switch (s) {
    ...
    case "accepted":  return "bg-[#c9a84c]/10 text-[#c9a84c] border-[#c9a84c]/20";
    ...
    <div className="bg-[#111] border border-[#1E1E1E] rounded-xl overflow-hidden">
      ...
      <div className="flex items-start justify-between gap-4 px-5 py-4 border-b border-[#1A1A1A]">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-0.5">{req.client_email} · {req.client_phone}</p>
          <p className="text-xs text-zinc-700 mt-0.5">
            {fmtDate(req.created_at)} · {artistName}
          </p>
        </div>
      ...
      {req.status === "quoted" && req.artist_note && (
        <div className="px-5 py-3 border-b border-[#1A1A1A] bg-green-500/[0.04]">
          <p className="text-xs uppercase tracking-widest text-green-400/70 mb-1">Artist Note to Client</p>
          <p className="text-sm text-zinc-300">{req.artist_note}</p>
        </div>
      )}
      {req.status === "declined" && req.declined_reason && (
        <div className="px-5 py-3 border-b border-[#1A1A1A] bg-red-500/[0.04]">
          ...
          <p className="text-sm text-zinc-300">{req.declined_reason}</p>
        </div>
      )}
...
export default function OwnerRequestsClient({
  ...
  function openDecline(id: string) {
    ...
    setModalReason(""); setModalError(null);
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {(["All", "Pending", "Approved", "Declined"] as const).map((label) => (
          <div key={label} className="bg-[#111] border border-[#1E1E1E] rounded-xl px-4 py-3 text-center">
            <p className="text-2xl font-semibold text-[#E8E8E8]">{counts[label]}</p>
            <p className="text-xs text-zinc-600 mt-0.5 uppercase tracking-wider">{label}</p>
          </div>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-[#111] border border-[#1E1E1E] rounded-xl px-6 py-16 text-center">
          <p className="text-zinc-500 text-sm">No requests match the selected filters.</p>
        </div>
      ) : (
            ...
            {modal.type === "approve" ? (
              <>
                ...
                <div>
                  ...
                  <textarea rows={3} value={modalNote}
              ...
              <>
                ...
                <div>
                  <label className="block text-xs uppercase tracking-widest text-zinc-500 mb-1.5">Reason *</label>
                  <textarea rows={4} value={modalReason}
                    onChange={(e) => setModalReason(e.target.value)}
                    className={`${inputCls} resize-none`}
                    placeholder="Let the client know why…" autoFocus
                  />
                </div>
                {modalError && <p className="text-red-400 text-xs">{modalError}</p>}
                <div className="flex gap-3">
