...
- **PRODUCTION STATUS:** see the session's final report for live-production retest confirmation after deploy.
- **TESTS:** `tests/unit/ai-consultation-chat-engine.test.ts` (13 mocked/deterministic cases — normal progression, multi-field extraction, vague answer, frustrated reply, anti-loop guard override, fallback field selection, completion gating, style normalization), `scripts/verify-consultation-anti-loop.mjs` (real browser, real Claude API, full multi-turn conversation against a disposable QA studio, refresh/persistence, conversation history integrity, owner cross-check) — both wired permanently into the QA Engine (`known-bug-regression` and `client` phases).
