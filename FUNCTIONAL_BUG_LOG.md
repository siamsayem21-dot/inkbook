...
## AI-CONSULT-001 (P1) — Client Portal AI Consultation gets stuck repeating the same question forever

- **PERSONA:** CLIENT
- **ROUTE:** `/portal/[studio]/consultation`
- **REPORTED BY:** Siam, live production testing, 2026-08-30.
- **ACTION:** Multi-turn AI consultation chat — client answers a question with real, extractable information (e.g. "tattoo on my left arm").
- **EXPECTED:** The AI extracts the answer, updates its internal state, and asks about the next genuinely missing field.
- **ACTUAL:** The AI ignores the answer and repeats "Thanks — could you tell me a bit more about what you're looking for?" (or, for some fields, the exact same field-specific question) forever, regardless of what the client says next — reproduced live on Siam's real broken conversation (`ai_chats.id=5c42cdd6-ab1c-427f-a239-4f0a26242414`, studio "Siam Enterprise").
- **ROOT CAUSE:** `lib/ai-consultation/chat-engine.ts` asked Claude to format every reply as JSON via a plain prompt instruction only, with no structural enforcement. Replaying the exact real broken conversation against the live Anthropic API 5 times, the model ignored the JSON instruction and replied in plain prose 4/5 times. Every prose reply fails `text.match(/\{[\s\S]*\}/)`, throws, and falls into a `catch` block that returns `gathered` completely unchanged. Once every REQUIRED field happened to already be filled, that fallback's own text was a dead-end generic message with no path back to progress — explaining the exact observed loop.
- **FIX:**
  1. `lib/ai/anthropic-provider.ts` / `lib/ai/types.ts` — `AIChatParams` gained an optional `tool` param; when set, the provider forces Anthropic's `tool_choice` (structurally guaranteed JSON, not merely requested) and returns the tool call's `input` instead of parsing free text.
  2. `lib/ai-consultation/chat-engine.ts` — the consultation turn now calls with a `consultation_turn` tool schema, and the model additionally self-reports `askingAbout` (which field, if any, its reply targets). A deterministic guard checks that self-report against the merged gathered state: if the model asks about a field that's already filled, the app overrides the reply with the actual next missing field (required first, then optional) instead of trusting the model's text — selected from data, not a scripted conversation. The `fallback()` path (API/parse failure) was also fixed to ask about the next real missing field, or wrap up and complete if literally nothing is missing, instead of the old dead-end generic message.
- **MULTI-TURN RESULT (isolated, exact real broken conversation replayed against the fix):** 8/8 and 3/3 clean extractions across two separate replay tests; a full 13-turn end-to-end replay (name → phone → multi-field → vague answer → style → color → budget → frustrated reply → 3 optional fields → completion) reached `complete=true` cleanly with zero repeats and zero fallback hits.
- **MULTI-TURN RESULT (real browser, pre-deploy):** the same live E2E script run against the still-undeployed production server correctly reproduced the original bug end-to-end (confirming the fix targets the real root cause, not a synthetic one) — the fix had not been deployed yet at that point.
- **TWO FOLLOW-UP BUGS FOUND DURING LIVE VERIFICATION (same investigation, both fixed and deployed before this entry was closed):**
  1. Forcing `tool_choice` guarantees the JSON *shape* matches the schema, not that the `complete` boolean stays semantically consistent with `reply`'s text. Live: the model wrote a correct, warm closing/recap message with `askingAbout=null` but left `complete: false`, stalling forever (every required field was already filled, so there was nothing left to extract on any future turn). Fixed: treat a reply that isn't asking about anything, once every required field is filled, as complete regardless of the model's own flag.
  2. The opposite failure, found immediately after fixing (1): the model set `complete: true` while its own `reply` was simultaneously asking a real, still-unanswered question ("Do you have any preferred dates... or are you flexible?", `askingAbout="preferredDates"`, not yet in `gathered`) — the app trusted the flag and submitted the consultation mid-question, cutting a legitimate in-progress conversation short. Fixed: never complete while genuinely still asking about a field that isn't filled yet, regardless of what `complete` says.
- **PRODUCTION STATUS:** CONFIRMED FIXED. Final live retest against the fully deployed fix (all three commits): 0 failures — normal progression, multi-field extraction, vague answer, frustrated reply (completed legitimately once required fields were done), conversation history integrity (0 verbatim-repeat assistant replies, 0 dead-end fallback messages), all 8 required fields correctly extracted, consultation correctly submitted (`ai_chats.status="submitted"`, linked `consultation_id`), owner's `/owner/consultations/[id]` page renders the correct client data, 0 console errors, 0 failed requests.
- **TESTS:** `tests/unit/ai-consultation-chat-engine.test.ts` (16 mocked/deterministic cases — normal progression, multi-field extraction, vague answer, frustrated reply, anti-loop guard override, fallback field selection, both completion-gating directions, style normalization), `scripts/verify-consultation-anti-loop.mjs` (real browser, real Claude API, full multi-turn conversation against a disposable QA studio, refresh/persistence, conversation history integrity, owner cross-check — resilient to legitimate early completion) — both wired permanently into the QA Engine (`known-bug-regression` and `client` phases).
- **STATUS:** FIXED, deployed, live-production retested clean.
